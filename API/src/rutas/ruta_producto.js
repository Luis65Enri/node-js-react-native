...
rutas.post(
  ...
  body("precio_producto")
    .isCurrency()
    .withMessage("Solo acepta valores decimales"),
  ...
  query("marcaId").isInt().withMessage("El id debe ser un numero entero"),
  controlador_producto.guardar
);
...
rutas.get(
  ...
  query("servicioId").isInt().withMessage("El id debe ser un numero entero"),
  query("marcaId").isInt().withMessage("El id debe ser un numero entero"),
  controlador_producto.busqueda
);
rutas.get(
  "/buscar=id",
  query("id").isInt().withMessage("El id debe ser un numero entero"),
  controlador_producto.busqueda_id
);
rutas.get(
  "/buscar=nombre",
  query("nombre").notEmpty().withMessage("Ingrese un valor en el nombre"),
  controlador_producto.busqueda_nombre
);
rutas.put(
  "/editarimagen",
  query("id")
    ...
    .custom(async (value) => {
      if (!value) {
        ...
        const buscaridProducto = await modelo_producto.findOne({
          where: { id: value },
        });
        if (!buscaridProducto) {
  ...
  controlador_producto.validarImagen,
  controlador_producto.actualizarimagenes
);
rutas.post(
  "/img",
  ValidarAutenticado,
  upload.single("img"),
  controlador_producto.GuardarImagen
);
