...
rutas.post(
  "/guardar",
  body("nombre_marca").notEmpty().withMessage("Ingrese un valor en el nombre"),
  controlador_marca.guardar
);
rutas.get("/listar", controlador_marca.listar);
rutas.put(
  "/editar",
  query("id").isInt().withMessage("El id debe ser un numero entero"),
  controlador_marca.editar
);
rutas.delete(
  "/eliminar",
  query("id").isInt().withMessage("El id debe ser un numero entero"),
  controlador_marca.eliminar
);
rutas.get(
  "/buscar",
  [
