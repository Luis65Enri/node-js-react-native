...
exports.editar = async (req, res) => {
  ...
  try {
    ...
    if (!buscar_proveedor) {
      ...
      buscar_proveedor.codigo_pais_telefono_proveedor =
        codigo_pais_telefono_proveedor;
      buscar_proveedor.telefono_proveedor = telefono_proveedor;
      await buscar_proveedor
        .save()
        .then((data) => {
          res.json(data);
        })
  ...
  if (validacion.errors.length > 0) {
    ...
    try {
      ...
      if (req.query.contacto)
        whereClause.contacto_proveedor = req.query.contacto;
      if (req.query.telefono)
        whereClause.telefono_proveedor = req.query.telefono;
...
exports.busqueda_nombre = async (req, res) => {
  ...
  if (validacion.errors.length > 0) {
    ...
    try {
      const busqueda = await modelo.findAll({
        where: { nombre_proveedor: req.query.nombre },
      });
      res.json(busqueda);
    } catch (error) {
      res.json(error);
