...
exports.busqueda_nombre = async(req,res)=>{
    ...
    if (validacion.errors.length > 0) {
        ...
        try{
            const busqueda = await modelo.findAll({where:{ nombre_producto:req.query.nombre}});
            res.json(busqueda)
        } catch (error) {
            res.json(error);
        }
        ...
        guardarImagenProducto(req, res, (err) => {
            if (err) {
                if (err instanceof multer.MulterError) {
                    return res.json({ msj: "Hay errores al cargar la imagen", error: err.message });
                } else {
                    return res.json({ msj: "Error al cargar la imagen", error: err.message });
                }
            } else {
                next(); 
            }
...
exports.actualizarimagen = async (req, res) => {
    ...
    if (validacion.errors.length > 0) {
        ...
        if(!buscar_producto){
            ...
            const imagenAnterior = fs.existsSync(path.join(__dirname, '../../public/img/productos/'+ buscar_producto.imagen));
            if(imagenAnterior){
                fs.unlinkSync(path.join(__dirname,'../../public/img/productos/'+ buscar_producto.imagen));
                console.log("Imagen eliminada");
            }
            const imagenNueva = fs.existsSync(path.join(__dirname, '../../public/img/productos/'+ nombreImagen));
            if (imagenNueva){
                buscar_producto.imagen = nombreImagen;
                buscar_producto.save()
                .then((data) => {
                    res.json(data);
                }).catch((er) => {
                    res.json(er);
...
exports.actualizarimagenes = async (req, res) => {
    ...
    if (validacion.errors.length > 0) {
        ...
        try {
            ...
            if (!producto) {
                return res.json({ msj: "El producto con el ID proporcionado no existe" });
            }
            if (producto.imagen) {
            ...
            return res.json({ msj: "Imagen actualizada exitosamente",nuevaImagen: nombreImagen });

        } catch (error) {
            console.error("Error al actualizar la imagen:", error);
            return res.json({ msj: "Error interno del servidor al actualizar la imagen" });
        }
    }
};
